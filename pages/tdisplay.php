...
class tDisplay extends forumPage {
	...
	public function breadCrumb() {
		...
		return $parentCrumb . " " . $breadcrumbs;
	}
	function getJS() {
		$highlighterRoot = "includes/packages/Syntax Highlighter/";
		$js = parent::getJS();
		$js[] = array( "path" => $highlighterRoot . "src/shCore.js" );
		...
		$js[] = array( "text" => "SyntaxHighlighter.all();" );
		return $js;
	}
	function getCSS() {
		$highlighterRoot = "includes/packages/Syntax Highlighter/";
	...
	public function display() {
		...
		if( !$GLOBALS['super']->user->can( "ViewBoard" ) || !$GLOBALS['super']->user->can( "Forum" . $this->currentForum['id'], "View" ) ) {
			...
			if ($GLOBALS['super']->user->can("Forum".$this->currentForum['id'],"Reply")){
				$forumActions[] = array(
						'url' =>	"index.php?act=topicReply&amp;id=".$this->currentTopic['id'],
						'name'	=>	"Post Reply"
					);
			}
			$topBar->add("LINKS", $forumActions);
			$totalPages = "SELECT count(id) as count FROM ".TBL_PREFIX."posts WHERE `topic_id`=".$this->currentTopic['id'];
			...
			if ($GLOBALS['super']->db->getRowCount($posts) > 0){
				...
				while($post = $GLOBALS['super']->db->fetch_assoc($posts)){
					...
					if ($post['last_edited'] != 0)
						...
						$editUser = $post['last_edited_user'];
					$canEdit = false;
